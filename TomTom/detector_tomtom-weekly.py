...
df.to_csv('detectors.csv')

loop_detector = []
for idx in range(len(df)):
    sum_detectors = float(df.iloc[idx, 0]) + float(df.iloc[idx, 1])
...
ratio = [tom/detector * 100 for tom, detector in zip(tomtom, loop_detector)]
print(ratio)

plt.plot(ratio, "b", label="Ratio of TomTom to Detector Data [%]")
# plt.plot(tomtom, "r", label="TomTom [probe size]")
plt.legend(loc="upper right")
plt.xlabel("hour")
plt.show()
